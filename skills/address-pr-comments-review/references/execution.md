...
This brief is bounded by a total Section A hard cap of 5, an ordered-chain count cap of 1, and an ordered-chain hard cap of 3 tasks; all remaining tasks must be independent singletons. Before execution, validate every task's `mechanical` or `local-behavior` complexity certificate, including its canonical hard-blocker evidence, and require `Hard blocker result: none`. Recompute the deterministic topological order from dependency edges, final-table concern order, and numeric task-ID tie-break, then require it to match the brief. Execute that order always serially, without another approval step. First validate checkout root, branch, HEAD, and PR identity; then for each task execute exactly: edit -> verify -> commit -> push -> remote-reachability -> reply -> read-back. Before each reply, require and validate `source_comment_id`, `root_comment_id`, `comment_kind`, `reply_mode`, `endpoint`, and `read_back_endpoint`. POST at most once, then read back remote state; on resume, reconcile prior writes before deciding whether any POST remains. Use a distinct task-specific commit SHA in each required reply. Apply the Direct Fix Failure Scope Matrix in the brief. At a proven safe checkpoint, a terminal task-local failure marks dependency-affected tasks blocked with failed prerequisite IDs while independent ready tasks continue serially. When no ready work remains and required blocked tasks exist, mark the artifact blocked. Global checkout, certificate, topology, or order failures, an unsafe checkpoint, or an unreconciled external write immediately block the artifact and permit no later task side effects. For an uncertain POST, only exactly one route-specific read-back match reconciles the write; zero, multiple, malformed, or incomplete read-back never authorizes another POST or resume until external state is resolved. Preserve completed evidence, do not reset, revert, restore, or discard pre-existing/user changes, and transition to `verified-complete` only after all required work and replies are verified. Do not expand scope beyond the brief or produce planning output.
```
