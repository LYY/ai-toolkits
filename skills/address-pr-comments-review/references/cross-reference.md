...
- **Related detection**: causally or logically connected comments across different files and lines
- **Already-replied detection**: two-level assessment -- has a reply vs reply is sufficient
- **Cross-file escalation**: detecting when a concern in one file is part of a broader pattern across the codebase
...
- **Escalation metadata**: how cross-file patterns are documented in the dossier

## Out of Scope

- Individual comment classification (source, intent, conclusion) -> `classification.md`
- Interactive discussion of conflicts or escalation -> `interaction.md`
- Dossier section assignment for merged entries -> `dossier.md` (consumes cross-reference output)
- Automated cross-file clustering scripts -> agent performs manual targeted search
- Full repo-wide static analysis -> cross-file escalation is pattern detection through targeted search, not analysis
...
| Conflict | Opposing recommendations on same code | Merge into one entry, flag for discussion, present both options |
| Related | Causal or logical connection across files | Note relationship, add dependency metadata |
| Already-replied | `has_replies: true` from `list_comments.py` | Two-level assessment: has a reply vs reply is sufficient. Default to `already_replied`, flag if insufficient. |
| Cross-file escalation | Targeted grep/search confirms same pattern in N other files | Evidence-gated: weak = observe, moderate = guardrail, strong = full escalation with dossier section |
