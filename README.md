...
| [address-pr-comments-review](./skills/address-pr-comments-review/) | 交互式 PR review 处理：绑定 checkout 后验证 PR，分类确认后生成 local artifacts 交接执行，支持 bounded Direct Fix、按 comment kind 路由的 direct reply 和 status-based resume。内置去重、冲突检测、Scope Guardrails、回复验证和 cleanup gates |
...
- **direct reply**: 无代码变更的 inline、review-level 或 top-level 回复按 comment kind 选择 root `/replies` 或 issue timeline endpoint，POST body 仅含 `body`，再按 route-specific read-back 验证；Direct Fix 仅适用于 1 至 5 个独立、低风险、机械性任务，每个任务独立 commit，失败即停止批次
