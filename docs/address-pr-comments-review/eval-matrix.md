...
> **canonical scenario corpus** for the address-pr-comments-review skill. This file is the authoritative source for the seven regression scenarios and QA tokens referenced by `output.md`.
>
...
**Token-to-conclusion mapping:** The `partial fix` scenario token maps to the `partially_addressed` classification conclusion defined in `analyze.md`.
