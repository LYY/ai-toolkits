...
> **⚠️ 编辑 skill 文件时，只改仓库源码（`skills/<skill-name>/`），不要改安装副本（`~/.agents/skills/<skill-name>/`）。**
>
> `skill` 工具加载 skill 时会返回安装目录路径（如 `~/.agents/skills/xxx/`），后续 `read`/`edit` 容易惯性使用这个路径——但安装副本不在 git 仓库内，改动不会被 tracking。始终确认所编辑的文件在仓库的 `skills/` 下。
...
- [ ] **流程/路径清晰度** — 所有分支是否覆盖？每条路径是否可达？有没有"走到一半断了"的分支？
- [ ] **Agent 导航性** — agent 仅凭 On-Demand Loading 表 + Workflow 图，能否找到每一步要读的文件？是否需要靠"猜测"跳转？
- [ ] **简洁性** — 有没有冗余描述、重复规则、过时文案？修改后保留旧逻辑了吗？
- [ ] **二义性** — 同一概念在不同文件里含义是否一致？"Step X"这样的编号在分支路径里是否还成立？
- [ ] **步骤编号一致性** — 分支路径有没有引用不存在的步骤编号？（如 reply-only 路径写 `Step 4c` 但 Step 4 根本没发生）
- [ ] **加载指引准确性** — On-Demand Loading 表覆盖所有路径了吗？跨文件引用背后是否真的满足自包含原则（原则 5）？
- [ ] **验证规则自包含** — 每个 reference 文件里的 Gate/Check 规则是否独立可读，不依赖未加载文件的内容？
